 ...
 * Chronological ledger: purchases (debit/payable+) + payments (credit/payable−).
 * Purchase rows come from existing stock docs — purchase logic unchanged.
 */
const ledger = async (req, res) => {
  try {
    ...
    const [rmPurchases, productPurchases, payments] = await Promise.all([
